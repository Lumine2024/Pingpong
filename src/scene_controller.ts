...
import { clamp, clampMagnitude, distanceSquared, scale, vecAdd, vecSub, Vector } from "./mathutil";
import { CONFIG } from "./config";

export interface SceneResetState {
...
export class SceneController {
    ...
    private rect1Velocity: Vector;
    private rect2Velocity: Vector;
    constructor() {
    ...
    private static createInitialBallVelocity(): Vector {
    ...
    reset(state: SceneResetState): void {
    ...
    getRect1Velocity(): Vector {
        return this.rect1Velocity;
    }
    getRect2Velocity(): Vector {
        return this.rect2Velocity;
    }
    setRect1Velocity(velocity: Vector): void {
        this.rect1Velocity = clampMagnitude(velocity, CONFIG.MAX_RECT_VELOCITY);
    }
    setRect2Velocity(velocity: Vector): void {
    ...
    private static clampPosition(position: Vector, upBound: number, downBound: number, leftBound: number, rightBound: number): Vector {
        return {
            x: clamp(position.x, leftBound, rightBound),
            y: clamp(position.y, upBound, downBound)
        };
    }
    private static clampRectPosition(position: Vector): Vector {
        return SceneController.clampPosition(position,
            0, CONFIG.STAGE_HEIGHT - CONFIG.RECT_HEIGHT,
            0, CONFIG.STAGE_WIDTH - CONFIG.RECT_WIDTH
        );
    }
    private static isBallCollidingRect(ballPosition: Vector, rectPosition: Vector): boolean {
        const closestPoint = SceneController.clampPosition(
            ballPosition,
            rectPosition.y, rectPosition.y + CONFIG.RECT_HEIGHT,
            rectPosition.x, rectPosition.x + CONFIG.RECT_WIDTH
        );
        return distanceSquared(ballPosition, closestPoint) <= CONFIG.BALL_RADIUS * CONFIG.BALL_RADIUS;
    }
    private applyRectCollision(rectVelocity: Vector, hitRect: "rect1" | "rect2"): void {
    ...
    gameLoop(): number {
        ...
        const boundedRect1Position = SceneController.clampPosition(
            nextRect1Position,
            0, CONFIG.STAGE_HEIGHT - CONFIG.RECT_HEIGHT,
            0, CONFIG.STAGE_WIDTH / 3
        );
        ...
        const boundedRect2Position = SceneController.clampPosition(
            nextRect2Position,
            0, CONFIG.STAGE_HEIGHT - CONFIG.RECT_HEIGHT,
            CONFIG.STAGE_WIDTH * 2 / 3, CONFIG.STAGE_WIDTH - CONFIG.RECT_WIDTH
        );
